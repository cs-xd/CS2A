...
csaa_seqio_t *csaa_bam_open(const char *fn, int which)
{
	csaa_seqio_t *bs;
	bam_header_t *h;
	bs = (csaa_seqio_t*)calloc(1, sizeof(csaa_seqio_t));
	bs->is_bam = 1;
	bs->which = which;
	bs->fp = bam_open(fn, "r");
	if (0 == bs->fp)
...
int csaa_trim_read(int trim_qual, csaa_seq_t *p)
{
	int s = 0, l, max = 0, max_l = p->len;
	if (trim_qual < 1 || p->qual == 0)
        return 0;
	for (l = p->len - 1; l >= CSAA_MIN_RDLEN; --l) {
...
static csaa_seq_t *csaa_read_bam(csaa_seqio_t *bs, int n_needed, int *n, int is_comp, int trim_qual)
{
	csaa_seq_t *seqs, *p;
	int n_seqs, l, i;
	...
	b = bam_init1();
	n_seqs = 0;
	seqs = (csaa_seq_t*)calloc(n_needed, sizeof(csaa_seq_t));
	while ((res = bam_read1(bs->fp, b)) >= 0) {
		...
		if (trim_qual >= 1)
            n_trimmed += csaa_trim_read(trim_qual, p);
	...
	*n = n_seqs;
	if (n_seqs && trim_qual >= 1)
		fprintf(stderr, "[csaa_read_seq] %.1f%% bases are trimmed.\n", 100.0f * n_trimmed/n_tot);
	if (n_seqs == 0) {
		free(seqs);
		bam_destroy1(b);
		return 0;
...
csaa_seq_t *csaa_read_seq(csaa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual)
{
	csaa_seq_t *seqs, *p;
	kseq_t *seq = bs->ks;
	int n_seqs, l, i, is_comp = mode&CSAA_MODE_COMPREAD, is_64 = mode&CSAA_MODE_IL13, l_bc = mode>>24;
	long n_trimmed = 0, n_tot = 0;

	if (l_bc > CSAA_MAX_BCLEN) {
		fprintf(stderr, "[%s] the maximum barcode length is %d.\n", __func__, CSAA_MAX_BCLEN);
		return 0;
	}
	if (bs->is_bam)
        return csaa_read_bam(bs, n_needed, n, is_comp, trim_qual); // l_bc has no effect for BAM input
	n_seqs = 0;
	seqs = (csaa_seq_t*)calloc(n_needed, sizeof(csaa_seq_t));
	while ((l = kseq_read(seq)) >= 0) {
		if ((mode & CSAA_MODE_CFY) && (seq->comment.l != 0)) {
