	...
	if (set_main) {
		...
		for (i = cnt = 0; i < n_aln; ++i) {
			...
			if (drand48() * (p->l - p->k + 1 + cnt) > (double)cnt) {
				...
				s->score = p->score;
				s->sa = p->k + (u64_t)((p->l - p->k + 1) * drand48());
			}
			cnt += p->l - p->k + 1;
		}
		s->c1 = cnt;
		...
		s->c2 = cnt - s->c1;
		s->type = s->c1 > 1? CSAA_TYPE_REPEAT : CSAA_TYPE_UNIQUE;
	}

	if (n_multi) {
		int64_t k, rest, n_occ, z = 0;
		for (k = n_occ = 0; k < n_aln; ++k) {
			const csa_aln1_t *q = aln + k;
			n_occ += q->l - q->k + 1;
		}
		if (s->multi)
            free(s->multi);
		...
		s->multi = (csa_multi1_t*)calloc(rest, sizeof(csa_multi1_t));
		for (k = 0; k < n_aln; ++k) {
			const csa_aln1_t *q = aln + k;
			if (q->l - q->k + 1 <= rest) {
				u64_t l;

//				fprintf(stderr,"[debug] k=%ld  l=%ld\n",q->k,q->l);
				for (l = q->k; l <= q->l; ++l) {
					s->multi[z].pos = l;
...
{
	csaa_aln2seq_core(n_aln, aln, s, 1, 0);
}
...
u64_t csaa_sa2pos(const bntseq_t *bns, const csa_t *csa, u64_t sapos, int ref_len, int *strand)
{
	u64_t pos_f;
	int is_rev;
	pos_f = csa_sa(csa, sapos); // position on the forward-reverse coordinate
...
void csaa_cal_pac_pos_core(const bntseq_t *bns, const csa_t *csa, csaa_seq_t *seq, const int max_mm, const float fnr)
{
	int max_diff=0;
	int strand=0;
	if (seq->type != CSAA_TYPE_UNIQUE && seq->type != CSAA_TYPE_REPEAT)
        return;
	max_diff = fnr > 0.0? csaa_cal_maxdiff(seq->len, CSAA_AVG_ERR, fnr) : max_mm;
	seq->seQ = seq->mapQ = csaa_approx_mapQ(seq, max_diff);
	//fprintf(stderr, "%d\n", seq->ref_shift);
	seq->pos = csaa_sa2pos(bns, csa, seq->sa, seq->len + seq->ref_shift, &strand);
	seq->strand = strand;
	seq->seQ = seq->mapQ = csaa_approx_mapQ(seq, max_diff);
	if (seq->pos == (u64_t)-1)
        seq->type = CSAA_TYPE_NO_MATCH;
}

void csaa_cal_pac_pos(const bntseq_t *bns, const char *prefix, int n_seqs, csaa_seq_t *seqs, int max_mm, float fnr)
{
	int i, j;
	int strand=0;
	int n_multi=0;
	...
	for (i = 0; i != n_seqs; ++i) {
		csaa_seq_t *p = &seqs[i];
		csaa_cal_pac_pos_core(bns, csa, p, max_mm, fnr);
		for (j = n_multi = 0; j < p->n_multi; ++j) {
			csa_multi1_t *q = p->multi + j;
			q->pos = csaa_sa2pos(bns, csa, q->pos, p->len + q->ref_shift, &strand);
			q->strand = strand;
			if (q->pos != p->pos && q->pos != (u64_t)-1)
				p->multi[n_multi++] = *q;
		}
		p->n_multi = n_multi;
	}
...
csaa_cigar_t *csaa_refine_gapped_core(u64_t l_pac, const ubyte_t *pacseq, int len, ubyte_t *seq, int ref_shift, u64_t *_rb, int *n_cigar)
{
...
char *csaa_cal_md1(int n_cigar, csaa_cigar_t *cigar, int len, u64_t pos, ubyte_t *seq,
				  u64_t l_pac, ubyte_t *pacseq, kstring_t *str, int *_nm)
{
	u64_t x, y;
	int z, u, c, nm = 0;
	str->l = 0; // reset
	...
	if (cigar) {
		...
		for (z = u = 0; z < (u64_t)len && x+z < l_pac; ++z) {
			c = pacseq[(x+z)>>2] >> ((~(x+z)&3)<<1) & 3;
			if (c > 3 || seq[y+z] > 3 || c != seq[y+z]) {
				ksprintf(str, "%d", u);
				kputc("ACGTN"[c], str);
...
void csaa_correct_trimmed(csaa_seq_t *s)
{
	if (s->len == s->full_len)
        return;
	if (s->strand == 0) { // forward
		if (s->cigar && __cigar_op(s->cigar[s->n_cigar-1]) == FROM_S) { // the last is S
			...
			if (s->cigar == 0) {
				s->n_cigar = 2;
				s->cigar = (csaa_cigar_t*)calloc(s->n_cigar, sizeof(csaa_cigar_t));
				s->cigar[0] = __cigar_create(0, s->len);
			}
            else {
				++s->n_cigar;
				s->cigar = (csaa_cigar_t*)realloc(s->cigar, s->n_cigar * sizeof(csaa_cigar_t));
			}
		...
		if (s->cigar && __cigar_op(s->cigar[0]) == FROM_S) { // the first is S
			...
			if (s->cigar == 0) {
				...
				++s->n_cigar;
				s->cigar = (csaa_cigar_t*)realloc(s->cigar, s->n_cigar * sizeof(csaa_cigar_t));
				memmove(s->cigar + 1, s->cigar, (s->n_cigar-1) * sizeof(csaa_cigar_t));
			}
...
void csaa_refine_gapped(const bntseq_t *bns, int n_seqs, csaa_seq_t *seqs, ubyte_t *_pacseq)
{
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		for (j = k = 0; j < s->n_multi; ++j) {
			csa_multi1_t *q = s->multi + j;
			int n_cigar;
			if (q->gap) { // gapped alignment
				q->cigar = csaa_refine_gapped_core(bns->l_pac, pacseq, s->len, q->strand? s->rseq : s->seq, q->ref_shift, &q->pos, &n_cigar);
				q->n_cigar = n_cigar;
				if (q->cigar)
                    s->multi[k++] = *q;
			}
		...
		if (s->type == CSAA_TYPE_NO_MATCH || s->type == CSAA_TYPE_MATESW || s->n_gapo == 0)
            continue;
		s->cigar = csaa_refine_gapped_core(bns->l_pac, pacseq, s->len, s->strand? s->rseq : s->seq, s->ref_shift, &s->pos, &s->n_cigar);
		if (s->cigar == 0)
            s->type = CSAA_TYPE_NO_MATCH;
	}
	// generate MD tag
	...
	for (i = 0; i != n_seqs; ++i) {
		csaa_seq_t *s = seqs + i;
		if (s->type != CSAA_TYPE_NO_MATCH) {
			int nm;
			s->md = csaa_cal_md1(s->n_cigar, s->cigar, s->len, s->pos, s->strand? s->rseq : s->seq, bns->l_pac, pacseq, str, &nm);
			s->nm = nm;
		}
	}
	free(str->s);
...
int64_t pos_end_multi(const csa_multi1_t *p, int len) // analogy to pos_end()
{
	if (p->cigar) {
		int j;
		int64_t x = p->pos;
...
static int64_t pos_5(const csaa_seq_t *p)
{
	if (p->type != CSAA_TYPE_NO_MATCH)
		return p->strand? pos_end(p) : p->pos;
	return -1;
}

void csaa_print_seq(FILE *stream, csaa_seq_t *seq) {
	char buffer[4096];
...
{
	int j;
	if (p->type != CSAA_TYPE_NO_MATCH || (mate && mate->type != CSAA_TYPE_NO_MATCH)) {
		...
		if (p->type != CSAA_TYPE_NO_MATCH && p->pos + j - bns->anns[seqid].offset > bns->anns[seqid].len)
		...
		if (p->strand)
            flag |= SAM_FSR;
		if (mate) {
			if (mate->type != CSAA_TYPE_NO_MATCH) {
				if (mate->strand)
                    flag |= SAM_FMR;
			}
            else
        ...
        else if (p->type == CSAA_TYPE_NO_MATCH)
            err_printf("*");
		else
            err_printf("%dM", p->len);

		// print mate coordinate
		if (mate && mate->type != CSAA_TYPE_NO_MATCH) {
			int m_seqid;
			long long isize;
		...
		if (p->type != CSAA_TYPE_NO_MATCH) {
			...
			if (nn > 10)
                XT = 'N';
			// print tags
			err_printf("\tXT:A:%c\t%s:i:%d", XT, (mode & CSAA_MODE_COMPREAD)? "NM" : "CM", p->nm);
			if (nn)
                err_printf("\tXN:i:%d", nn);
			if (mate)
                err_printf("\tSM:i:%d\tAM:i:%d", p->seQ, am);
			if (p->type != CSAA_TYPE_MATESW) { // X0 and X1 are not available for this type of alignment
...
void csaa_sai2sam_se_core(const char *prefix, const char *fn_sa, const char *fn_fa, int n_occ, const char *rg_line)
{
	...
	if (strncmp(magic, SAI_MAGIC, 4) != 0) {
		fprintf(stderr, "[E::%s] Unmatched SAI magic. Please re-run `aln' with the same version of csaa.\n", __func__);
		exit(1);
	}
	err_fread_noeof(&opt, sizeof(gap_opt_t), 1, fp_sa);
	csaa_print_sam_hdr(bns, rg_line);
	// set ks
	ks = csaa_open_reads(opt.mode, fn_fa);
	// core loop
	while ((seqs = csaa_read_seq(ks, 0x40000, &n_seqs, opt.mode, opt.trim_qual)) != 0) {
		tot_seqs += n_seqs;
		t = clock();
		...
		for (i = 0; i < n_seqs; ++i) {
			...
			if (n_aln > m_aln) {
				m_aln = n_aln;
				aln = (csa_aln1_t*)realloc(aln, sizeof(csa_aln1_t) * m_aln);
			}
			err_fread_noeof(aln, sizeof(csa_aln1_t), n_aln, fp_sa);
			//fprintf(stderr,"[debug] i=%d\n",i);
			csaa_aln2seq_core(n_aln, aln, p, 1, n_occ);
		}

		fprintf(stderr, "[csaa_aln_core] convert to sequence coordinate... ");
		csaa_cal_pac_pos(bns, prefix, n_seqs, seqs, opt.max_diff, opt.fnr); // forward bwt will be destroyed here
		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

//		fprintf(stderr, "[csaa_aln_core] refine gapped alignments... ");
		csaa_refine_gapped(bns, n_seqs, seqs, 0);
//		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

		fprintf(stderr, "[csaa_aln_core] out alignments... ");
		for (i = 0; i < n_seqs; ++i)
            csaa_print_sam1(bns, seqs + i, 0, opt.mode, opt.max_top2);
		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC);

		csaa_free_read_seq(n_seqs, seqs);
		fprintf(stderr, "[csaa_aln_core] %d sequences have been processed.\n", tot_seqs);
	}
...
int csaa_sai2sam_se(int argc, char *argv[])
{
	int c, n_occ = 3;
	...
	while ((c = getopt(argc, argv, "hn:f:r:")) >= 0) {
		...
		case 'h': break;
		case 'r':
			if ((rg_line = csaa_set_rg(optarg)) == 0)
                return 1;
			break;
	...
	if (optind + 3 > argc) {
		fprintf(stderr, "Usage: csaa se [-n max_occ] [-f out.sam] <prefix> <in.sai> <in.fq>\n");
		return 1;
	}
	...
	csaa_sai2sam_se_core(prefix, argv[optind+1], argv[optind+2], n_occ, rg_line);
	free(prefix);
	return 0;
}
