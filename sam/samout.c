...
{
	return csaa_gen_cigar2(mat, q, r, q, r, w_, l_pac, pac, l_query, query, rb, re, score, n_cigar, NM);
}
...
csa_t *csaa_idx_load_csa(const char *hint)
{
	char *tmp, *prefix;
	csa_t *csa;
	prefix = csaa_idx_infer_prefix(hint);
	if (prefix == 0) {
		if (csaa_verbose >= 1)
	...
	strcat(strcpy(tmp, prefix), ".csa");
	csa = csa_restore(tmp);
//	strcat(strcpy(tmp, prefix), ".sa");  // partial suffix array (SA)
//	csa_restore_sa(tmp, csa);
	free(tmp);
    free(prefix);
	return csa;
}
...
csaaidx_t *csaa_idx_load(const char *hint, int which)
{
	...
	idx = (csaaidx_t*)calloc(1, sizeof(csaaidx_t));
	if (which & CSAA_IDX_CSA)
        idx->csa = csaa_idx_load_csa(hint);
	if (which & CSAA_IDX_BNS) {
		idx->bns = bns_restore(prefix);
		if (which & CSAA_IDX_PAC) {
...
static char *csaa_escape(char *s)
{
	char *p, *q;
	for (p = q = s; *p; ++p) {
		if (*p == '\\') {
...
char *csaa_set_rg(const char *s)
{
	...
	if ((p = strstr(rg_line, "\tID:")) == 0) {
		if (csaa_verbose >= 1)
            fprintf(stderr, "[E::%s] no ID at the read group line\n", __func__);
		goto err_set_rg;
	}
	p += 4;
	for (q = p; *q && *q != '\t' && *q != '\n'; ++q);
