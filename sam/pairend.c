...
typedef struct {
	...
	u64_t low, high, high_bayesian;
} isize_info_t;

#define b128_eq(a, b) ((a).x == (b).x && (a).y == (b).y)
#define b128_hash(a) ((uint32_t)(a).x)

#include "../kseq/khash.h"
KHASH_INIT(b128, pair64_t, poslist_t, 1, b128_hash, b128_eq)

typedef struct {
	pair64_v arr;
	pair64_v pos[2];
	kvec_t(csa_aln1_t) aln[2];
} pe_data_t;

#define MIN_HASH_WIDTH 1000

extern int g_log_n[256]; // in singleend.c.
static kh_b128_t *g_hash;

void csaa_aln2seq_core(int n_aln, const csa_aln1_t *aln, csaa_seq_t *s, int set_main, int n_multi);
void csaa_aln2seq(int n_aln, const csa_aln1_t *aln, csaa_seq_t *s);
int csaa_approx_mapQ(const csaa_seq_t *p, int mm);
void csaa_print_sam1(const bntseq_t *bns, csaa_seq_t *p, const csaa_seq_t *mate, int mode, int max_top2);
bntseq_t *csaa_open_nt(const char *prefix);
void csaa_print_sam_SQ(const bntseq_t *bns);

pe_opt_t *csaa_init_pe_opt()
{
	pe_opt_t *po;
...
static int infer_isize(int n_seqs, csaa_seq_t *seqs[2], isize_info_t *ii, double ap_prior, int64_t L)
{
	...
	ii->high_bayesian = (u64_t)(y * ii->std + ii->avg + .499);
	for (i = 0; i < tot; ++i)
...
static int pairing(csaa_seq_t *p[2], pe_data_t *d, const pe_opt_t *opt, int s_mm, const isize_info_t *ii)
{
...
#define __pairing_aux2(q, w) do { \
		const csa_aln1_t *r = d->aln[(w).y&1].a + ((w).y>>2); \
...
typedef struct {
	kvec_t(csa_aln1_t) aln;
} aln_buf_t;

int csaa_cal_pac_pos_pe(const bntseq_t *bns, const char *prefix, csa_t *const _csa, int n_seqs, csaa_seq_t *seqs[2], FILE *fp_sa[2], isize_info_t *ii,
					   const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii)
{
	...
	if (_csa == 0) { // load forward SA
		strcpy(str, prefix); strcat(str, ".csa");  csa = csa_restore(str);
//		strcpy(str, prefix); strcat(str, ".sa"); csa_restore_sa(csa, str);
	}
    else
        csa = _csa;
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		for (j = 0; j < 2; ++j) {
			...
			if (n_aln > kv_max(d->aln[j]))
				kv_resize(csa_aln1_t, d->aln[j], n_aln);
			d->aln[j].n = n_aln;
			err_fread_noeof(d->aln[j].a, sizeof(csa_aln1_t), n_aln, fp_sa[j]);
			kv_copy(csa_aln1_t, buf[j][i].aln, d->aln[j]); // backup d->aln[j]
			// generate SE alignment and mapping quality
			csaa_aln2seq(n_aln, d->aln[j].a, p[j]);
			if (p[j]->type == CSAA_TYPE_UNIQUE || p[j]->type == CSAA_TYPE_REPEAT) {
				int strand;
				int max_diff = gopt->fnr > 0.0? csaa_cal_maxdiff(p[j]->len, CSAA_AVG_ERR, gopt->fnr) : gopt->max_diff;
				p[j]->seQ = p[j]->mapQ = csaa_approx_mapQ(p[j], max_diff);
				p[j]->pos = csaa_sa2pos(bns, csa, p[j]->sa, p[j]->len + p[j]->ref_shift, &strand);
				p[j]->strand = strand;
				if (p[j]->pos == (u64_t)-1)
                    p[j]->type = CSAA_TYPE_NO_MATCH;
			}
		}
	}

	// infer isize
	infer_isize(n_seqs, seqs, ii, opt->ap_prior, csa->csa->GetN()/2);
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		for (j = 0; j < 2; ++j) {
			p[j] = seqs[j] + i;
			kv_copy(csa_aln1_t, d->aln[j], buf[j][i].aln);
		}
		if ((p[0]->type == CSAA_TYPE_UNIQUE || p[0]->type == CSAA_TYPE_REPEAT)
			&& (p[1]->type == CSAA_TYPE_UNIQUE || p[1]->type == CSAA_TYPE_REPEAT))
				...
				for (k = 0; k < d->aln[j].n; ++k) {
					...
					if (0 && r->l - r->k + 1 >= MIN_HASH_WIDTH) { // then check hash table
						...
						if (ret) { // not in the hash table; ret must equal 1 as we never remove elements
							...
							z->n = r->l - r->k + 1;
							z->a = (u64_t*)malloc(sizeof(u64_t) * z->n);
							for (l = r->k; l <= r->l; ++l) {
								int strand;
								z->a[l - r->k] = csaa_sa2pos(bns, csa, l, p[j]->len + p[j]->ref_shift, &strand)<<1;
								z->a[l - r->k] |= strand;
							}
						}
						...
						for (l = r->k; l <= r->l; ++l) {
							int strand;
							x.x = csaa_sa2pos(bns, csa, l, p[j]->len + p[j]->ref_shift, &strand);
							x.y = k<<2 | strand<<1 | j;
							kv_push(pair64_t, d->arr, x);
						}
					}
		...
		if (opt->N_multi || opt->n_multi) {
			for (j = 0; j < 2; ++j) {
				if (p[j]->type != CSAA_TYPE_NO_MATCH) {
					int k, n_multi;
					if (!(p[j]->extra_flag&SAM_FPP) && p[1-j]->type != CSAA_TYPE_NO_MATCH) {
						csaa_aln2seq_core(d->aln[j].n, d->aln[j].a, p[j], 0, p[j]->c1+p[j]->c2-1 > opt->N_multi? opt->n_multi : opt->N_multi);
					}
                    else
                        csaa_aln2seq_core(d->aln[j].n, d->aln[j].a, p[j], 0, opt->n_multi);
					for (k = 0, n_multi = 0; k < p[j]->n_multi; ++k) {
						int strand;
						csa_multi1_t *q = p[j]->multi + k;
						q->pos = csaa_sa2pos(bns, csa, q->pos, p[j]->len + q->ref_shift, &strand);
						q->strand = strand;
						if (q->pos != p[j]->pos)
							p[j]->multi[n_multi++] = *q;
					}
...
csaa_cigar_t *csaa_sw_core(u64_t l_pac, const ubyte_t *pacseq, int len, const ubyte_t *seq, int64_t *beg, int reglen, int *n_cigar, uint32_t *_cnt)
{
	kswr_t r;
	uint32_t *cigar32 = 0;
	csaa_cigar_t *cigar = 0;
	ubyte_t *ref_seq;
	u64_t k, x, y, l;
	int xtra, gscore;
	int8_t mat[25];

	csaa_fill_scmat(1, 3, mat);
	...
	{ // update cigar and coordinate;
		...
		*beg += r.tb;
		cigar = (csaa_cigar_t*)realloc(cigar, sizeof(csaa_cigar_t) * (*n_cigar + 2));
		if (start) {
			memmove(cigar + 1, cigar, sizeof(csaa_cigar_t) * (*n_cigar));
...
ubyte_t *csaa_paired_sw(const bntseq_t *bns, const ubyte_t *_pacseq, int n_seqs, csaa_seq_t *seqs[2], const pe_opt_t *popt, const isize_info_t *ii)
{
	ubyte_t *pacseq;
	int i;
...
#define __set_fixed(_pref, _pmate, _beg, _cnt) do {						\
				_pmate->type = CSAA_TYPE_MATESW;							\
				_pmate->pos = _beg;										\
				_pmate->seQ = _pref->seQ;								\
				_pmate->strand = (popt->type == CSAA_PET_STD)? 1 - _pref->strand : _pref->strand; \
			...
			is_singleton = (p[0]->type == CSAA_TYPE_NO_MATCH || p[1]->type == CSAA_TYPE_NO_MATCH)? 1 : 0;

			++n_tot[is_singleton];
			cigar[0] = cigar[1] = 0;
			n_cigar[0] = n_cigar[1] = 0;
			if (popt->type != CSAA_PET_STD) continue; // other types of pairing is not considered
			for (k = 0; k < 2; ++k) { // p[1-k] is the reference read and p[k] is the read considered to be modified
				ubyte_t *seq;
				if (p[1-k]->type == CSAA_TYPE_NO_MATCH)
				...
				cigar[k] = csaa_sw_core(bns->l_pac, pacseq, p[k]->len, seq, &beg[k], end[k] - beg[k], &n_cigar[k], &cnt[k]);
				if (cigar[k] && p[k]->type != CSAA_TYPE_NO_MATCH) { // re-evaluate cigar[k]
	...
	fprintf(stderr, "[csaa_paired_sw] %lld out of %lld Q%d singletons are mated.\n",
			(long long)n_mapped[1], (long long)n_tot[1], SW_MIN_MAPQ);
	fprintf(stderr, "[csaa_paired_sw] %lld out of %lld Q%d discordant pairs are fixed.\n",
			(long long)n_mapped[0], (long long)n_tot[0], SW_MIN_MAPQ);
	return pacseq;
}

void csaa_sai2sam_pe_core(const char *prefix, char *const fn_sa[2], char *const fn_fa[2], pe_opt_t *popt, const char *rg_line)
{
	extern csaa_seqio_t *csaa_open_reads(int mode, const char *fn_fa);
	...
	// initialization
	csaase_initialize(); // initialize g_log_n[] in singleend.c
	pac = 0;
    csa = 0;
	for (i = 1; i != 256; ++i)
	...
	if (strncmp(magic[0], SAI_MAGIC, 4) != 0 || strncmp(magic[1], SAI_MAGIC, 4) != 0) {
		fprintf(stderr, "[E::%s] Unmatched SAI magic. Please rebuld the index.\n", __func__);
		exit(1);
	}
	err_fread_noeof(&opt, sizeof(gap_opt_t), 1, fp_sa[0]);
	ks[0] = csaa_open_reads(opt.mode, fn_fa[0]);
	opt0 = opt;
	err_fread_noeof(&opt, sizeof(gap_opt_t), 1, fp_sa[1]); // overwritten!
	ks[1] = csaa_open_reads(opt.mode, fn_fa[1]);
	...
	// core loop
	csaa_print_sam_hdr(bns, rg_line);
	while ((seqs[0] = csaa_read_seq(ks[0], 0x40000, &n_seqs, opt0.mode, opt0.trim_qual)) != 0) {
		int cnt_chg;
		isize_info_t ii;
		ubyte_t *pacseq;

		seqs[1] = csaa_read_seq(ks[1], 0x40000, &n_seqs, opt.mode, opt.trim_qual);
		tot_seqs += n_seqs;
		t = clock();

		fprintf(stderr, "[csa_sai2sam_pe_core] convert to sequence coordinate... \n");
		cnt_chg = csaa_cal_pac_pos_pe(bns, prefix, csa, n_seqs, seqs, fp_sa, &ii, popt, &opt, &last_ii);
		fprintf(stderr, "[csaa_sai2sam_pe_core] time elapses: %.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
		fprintf(stderr, "[csaa_sai2sam_pe_core] changing coordinates of %d alignments.\n", cnt_chg);

		fprintf(stderr, "[csaa_sai2sam_pe_core] align unmapped mate...\n");
		pacseq = csaa_paired_sw(bns, pac, n_seqs, seqs, popt, &ii);
		fprintf(stderr, "[csaa_sai2sam_pe_core] time elapses: %.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

		fprintf(stderr, "[csaa_sai2sam_pe_core] refine gapped alignments... ");
		for (j = 0; j < 2; ++j)
			csaa_refine_gapped(bns, n_seqs, seqs[j], pacseq);
		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
		if (pac == 0)
            free(pacseq);

		fprintf(stderr, "[csaa_sai2sam_pe_core] print alignments... ");
		for (i = 0; i < n_seqs; ++i) {
			csaa_seq_t *p[2];
			p[0] = seqs[0] + i;
            p[1] = seqs[1] + i;
			...
			csaa_print_sam1(bns, p[0], p[1], opt.mode, opt.max_top2);
			csaa_print_sam1(bns, p[1], p[0], opt.mode, opt.max_top2);
		...
		for (j = 0; j < 2; ++j)
			csaa_free_read_seq(n_seqs, seqs[j]);
		fprintf(stderr, "[csaa_sai2sam_pe_core] %d sequences have been processed.\n", tot_seqs);
		last_ii = ii;
	}
...
int csaa_sai2sam_pe(int argc, char *argv[])
{
	int c;
	pe_opt_t *popt;
	char *prefix, *rg_line = 0;

	popt = csaa_init_pe_opt();
	while ((c = getopt(argc, argv, "a:o:sPn:N:c:f:Ar:")) >= 0) {
		switch (c) {
		case 'r':
			if ((rg_line = csaa_set_rg(optarg)) == 0) return 1;
			break;
	...
	if (optind + 5 > argc) {
		fprintf(stderr, "\n");
		fprintf(stderr, "Usage:   csaa pe [options] <prefix> <in1.sai> <in2.sai> <in1.fq> <in2.fq>\n\n");
	...
	if ((prefix = csaa_idx_infer_prefix(argv[optind])) == 0) {
		fprintf(stderr, "[%s] fail to locate the index\n", __func__);
		return 1;
	}
	csaa_sai2sam_pe_core(prefix, argv + optind + 1, argv + optind+3, popt, rg_line);
	free(prefix);
    free(popt);
	return 0;
}
