...
gap_opt_t *gap_init_opt()
{
	...
	o->mode = CSAA_MODE_GAPE | CSAA_MODE_COMPREAD;
	o->seed_len = 32;
    o->max_seed_diff = 2;
	o->fnr = 0.04;
	o->n_threads = 1;
...
int csa_cal_width( csa_t *csa, int len, const ubyte_t *str, csa_width_t *width)
{
	u64_t k, l, ok, ol;
	int i, bid;
	bid = 0;
...
void csaa_cal_sa_reg_gap(int tid, csa_t *const csa, int n_seqs, csaa_seq_t *seqs, const gap_opt_t *opt)
{
	...
	for (i = max_len = 0; i != n_seqs; ++i)
		if (seqs[i].len > max_len)
            max_len = seqs[i].len;
	if (opt->fnr > 0.0)
        local_opt.max_diff = csaa_cal_maxdiff(max_len, CSAA_AVG_ERR, opt->fnr);
	...
	seed_w = (csa_width_t*)calloc(opt->seed_len+1, sizeof(csa_width_t));
	w = 0;
	for (i = 0; i != n_seqs; ++i) {
		csaa_seq_t *p = seqs + i;
#ifdef HAVE_PTHREAD
        ...
        if (max_l < p->len) {
			max_l = p->len;
			w = (csa_width_t*)realloc(w, (max_l + 1) * sizeof(csa_width_t));
			memset(w, 0, (max_l + 1) * sizeof(csa_width_t));
		}
		csa_cal_width(csa, p->len, p->seq, w);
		if (opt->fnr > 0.0)
            local_opt.max_diff = csaa_cal_maxdiff(p->len, CSAA_AVG_ERR, opt->fnr);
		local_opt.seed_len = opt->seed_len < p->len? opt->seed_len : 0x7fffffff;
		if (p->len > opt->seed_len)
			csa_cal_width(csa, opt->seed_len, p->seq + (p->len - opt->seed_len), seed_w);
		// core function
		for (j = 0; j < p->len; ++j)
			p->seq[j] = p->seq[j] > 3? 4 : 3 - p->seq[j];
		p->aln = csa_match_gap(csa, p->len, p->seq, w, p->len <= opt->seed_len? 0 : seed_w, &local_opt, &p->n_aln, stack);
...
static void *worker(void *data)
{
	...
	csaa_cal_sa_reg_gap(d->tid, d->csa, d->n_seqs, d->seqs, d->opt);
	return 0;
}
#endif

csaa_seqio_t *csaa_open_reads(int mode, const char *fn_fa)
{
	csaa_seqio_t *ks;
	if (mode & CSAA_MODE_BAM) { // open BAM
		int which = 0;
		if (mode & CSAA_MODE_BAM_SE)
            which |= 4;
		if (mode & CSAA_MODE_BAM_READ1)
            which |= 1;
		if (mode & CSAA_MODE_BAM_READ2)
            which |= 2;
		if (which == 0)
            which = 7; // then read all reads
		ks = csaa_bam_open(fn_fa, which);
	} else
        ks = csaa_seq_open(fn_fa);
	return ks;
}
...
void csaa_aln_core(const char *prefix, const char *fn_fa, const gap_opt_t *opt)
{
	...
	while ((seqs = csaa_read_seq(ks, 0x40000, &n_seqs, opt->mode, opt->trim_qual)) != 0) {
		tot_seqs += n_seqs;
		t = clock();
		fprintf(stderr, "[alignment_core] calculate SA coordinate... ");
#ifdef HAVE_PTHREAD
		if (opt->n_threads <= 1) { // no multi-threading at all
			csaa_cal_sa_reg_gap(0, csa, n_seqs, seqs, opt);
		}
        else {
			pthread_t *tid;
			pthread_attr_t attr;
		...
		t = clock();
		fprintf(stderr, "[algnment_core] write to the disk... ");
		for (i = 0; i < n_seqs; ++i) {
			csaa_seq_t *p = seqs + i;
			err_fwrite(&p->n_aln, 4, 1, stdout);
			if (p->n_aln)
                err_fwrite(p->aln, sizeof(csa_aln1_t), p->n_aln, stdout);
		}
		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC);

		csaa_free_read_seq(n_seqs, seqs);
		fprintf(stderr, "[algnment_core] %d sequences have been processed.\n", tot_seqs);
	}
...
int csaa_aln(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "n:o:e:i:d:l:k:LR:m:t:NM:O:E:q:f:b012IYB:")) >= 0) {
		...
		case 'L': opt->mode |= CSAA_MODE_LOGGAP; break;
		case 'R': opt->max_top2 = atoi(optarg); break;
		case 'q': opt->trim_qual = atoi(optarg); break;
		case 'N': opt->mode |= CSAA_MODE_NONSTOP; opt->max_top2 = 0x7fffffff; break;
		case 'f': xreopen(optarg, "wb", stdout); break;
		case 'b': opt->mode |= CSAA_MODE_BAM; break;
		case '0': opt->mode |= CSAA_MODE_BAM_SE; break;
		case '1': opt->mode |= CSAA_MODE_BAM_READ1; break;
		case '2': opt->mode |= CSAA_MODE_BAM_READ2; break;
		case 'I': opt->mode |= CSAA_MODE_IL13; break;
		case 'Y': opt->mode |= CSAA_MODE_CFY; break;
	...
	if (opte > 0) {
		opt->max_gape = opte;
		opt->mode &= ~CSAA_MODE_GAPE;
	}

	if (optind + 2 > argc) {
		fprintf(stderr, "\n");
		fprintf(stderr, "Usage:   csaa aln [options] <prefix> <in.fq>\n\n");
		fprintf(stderr, "Options: -n NUM    max #diff (int) or missing prob under %.2f err rate (float) [%.2f]\n",
				CSAA_AVG_ERR, opt->fnr);
	...
	if (opt->fnr > 0.0) {
		...
		for (i = 17, k = 0; i <= 250; ++i) {
			int l = csaa_cal_maxdiff(i, CSAA_AVG_ERR, opt->fnr);
			if (l != k) fprintf(stderr, "[csaa_aln] %dbp reads: max_diff = %d\n", i, l);
			k = l;
		}
	}
	...
	csaa_aln_core(prefix, argv[optind+1], opt);
