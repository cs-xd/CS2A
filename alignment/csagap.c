...
static inline void gap_push(gap_stack_t *stack, int i, u64_t k, u64_t l, int n_mm, int n_gapo, int n_gape, int n_ins, int n_del,
...
static inline void gap_shadow(int x, int len, u64_t max, int last_diff_pos, csa_width_t *w)
{
	int i, j;
...
csa_aln1_t *csa_match_gap(csa_t *const csa, int len, const ubyte_t *seq, csa_width_t *width,
						  csa_width_t *seed_width, const gap_opt_t *opt, int *_n_aln, gap_stack_t *stack)
{ // $seq is the reverse complement of the input read
	...
	m_aln = 4; n_aln = 0;
	aln = (csa_aln1_t*)calloc(m_aln, sizeof(csa_aln1_t));
	...
	gap_push(stack, len, 0, csa->csa->GetN(), 0, 0, 0, 0, 0, 0, 0, opt);

	while (stack->n_entries) {
		gap_entry_t e;
		int i, m, m_seed = 0, hit_found, allow_diff, allow_M, tmp;
		u64_t k, l, cnt_k[4], cnt_l[4], occ;
		...
		if (!(opt->mode & CSAA_MODE_NONSTOP) && e.info>>21 > best_score + opt->s_mm)
            break; // no need to proceed

		m = max_diff - (e.n_mm + e.n_gapo);
		if (opt->mode & CSAA_MODE_GAPE)
            m -= e.n_gape;
		if (m < 0)
            continue;
		...
		if (i == 0)
            hit_found = 1;
		else if (m == 0 && (e.state == STATE_M || (opt->mode&CSAA_MODE_GAPE) || e.n_gape == opt->max_gape)) { // no diff allowed
			if (csa_match_exact_alt(csa, i, seq, &k, &l))
                hit_found = 1;
			else
		...
		if (hit_found) { // action for found hits
			...
			if (do_add) { // append
				csa_aln1_t *p;
				gap_shadow(l - k + 1, len, csa->csa->GetN(), e.last_diff_pos, width);
				if (n_aln == m_aln) {
					m_aln <<= 1;
					aln = (csa_aln1_t*)realloc(aln, m_aln * sizeof(csa_aln1_t));
					memset(aln + m_aln/2, 0, m_aln/2*sizeof(csa_aln1_t));
				}
				p = aln + n_aln;
				p->n_mm = e.n_mm;
                p->n_gapo = e.n_gapo;
        ...
        for(int i=0;i<4;i++)
        {
            csa->csa->boundary(k,l,cnt_k+i,cnt_l+i,i);
        }
		occ = l - k + 1;
		...
		// indels
		tmp = (opt->mode & CSAA_MODE_LOGGAP)? int_log2(e.n_gape + e.n_gapo)/2+1 : e.n_gapo + e.n_gape;
